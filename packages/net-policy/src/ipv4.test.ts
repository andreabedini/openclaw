...
describe("net-policy/ipv4", () => {
  ...
  it("validates canonical ipv4 and ipv6 custom bind addresses", () => {
    expect(validateIpAddressInput("192.168.1.100")).toBeUndefined();
    expect(validateIpAddressInput("::1")).toBeUndefined();
    expect(validateIpAddressInput("[::1]")).toBeUndefined();
    expect(validateIpAddressInput("2001:db8::1")).toBeUndefined();
    expect(validateIpAddressInput("example.com")).toBe(
      "Invalid IP address (e.g., 192.168.1.100 or ::1)",
    );
  });
});
