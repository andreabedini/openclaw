...
- **Docker note**: the default `loopback` bind listens on local-only interfaces (`127.0.0.1` / `::1`) inside the container. With Docker bridge networking (`-p 18789:18789`), traffic arrives on `eth0`, so the gateway is unreachable unless you publish non-loopback interfaces. Use `--network host`, or set `bind: "lan"` (or `bind: "custom"` with `customBindHost: "0.0.0.0"` or `"::"`).
